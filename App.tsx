...
const App: React.FC = () => {
    ...
    if (isLive && videoRef.current && liveCropRegion) {
      ...
      const captureAndProcess = async () => {
        ...
        if (ctx) {
           ...
           // Ngưỡng sai số (Tolerance)
           // Tăng lên 50-60 nếu chữ bị mất nét. Giảm xuống 30-40 nếu còn nhiều rác.
           const limit = 55; 
           ...
           for (let i = 0; i < data.length; i += 4) {
               ...
               // Khoảng cách tới màu TRẮNG (255, 255, 255)
               ...
               if (dist < limit) {
                   // Gần trắng (Chữ) -> Giữ nguyên TRẮNG (255, 255, 255)
                   data[i] = 255;
                   data[i + 1] = 255;
                   data[i + 2] = 255;
               } else {
                   // Không phải chữ -> Tô ĐEN (0, 0, 0) - Nền đen
                   data[i] = 0;
                   data[i + 1] = 0;
                   data[i + 2] = 0;
               }
  ...
  return (
    <div className={`min-h-screen flex flex-col ${settings.darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50'}`}>
      ...
      />
      {isLive && <div className="bg-red-600 text-white text-center text-sm py-1 font-medium animate-pulse">🔴 LIVE MODE: Tesseract OCR + Google Translate Free</div>}
